...
### D7. Load testing via JMeter over REST and browser bulk launcher
A JMeter `.jmx` plan issues `POST /orders` at a configurable request rate (thread group + constant throughput timer) to exceed 50 msg/s and generate lag. The `order-service` also serves a static `/load-generator/` page that accepts an order count, dispatches requests with bounded browser-side concurrency, and reports progress, successes, and failures. JMeter remains the reproducible, rate-controlled load source; the page is an interactive alternative for quickly creating a chosen batch. Serving it from `order-service` keeps the requests same-origin and avoids CORS configuration or an extra container. Alternative: JMeter Kafka plugins (pepper-box) producing directly to the broker — rejected as it bypasses the service and the produce-side trace; noted as optional.
